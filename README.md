...
> 이 게임은 비그래픽 가상 실험이다. 실제 사람, 실제 죽음, 실제 지급과 관계없다. 표시 인구와 모든 추첨 확률은 추정치다. 실제 이름·도시·개인정보를 생성하거나 사용하지 않는다. 대상군도 실제 범죄·수형자 데이터와 무관한 가상 조건이다. 이름은 실제 개인과 무관한 합성 가상 이름이다.
